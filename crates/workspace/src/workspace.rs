...
impl Workspace {
    ...
    pub fn join_all_panes(&mut self, pane: View<Pane>, cx: &mut ViewContext<Self>) {
        self.join_pane_into_next(pane.clone(), cx);
    }
...
mod tests {
    ...
    async fn test_join_all_panes(cx: &mut gpui::TestAppContext) {
        ...
        workspace.update(cx, |workspace, cx| {
            workspace.join_all_panes(workspace.active_pane().clone(), cx);
        });
